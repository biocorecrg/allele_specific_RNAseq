...
multiconfig = file("pre_dna_config.yaml")
...
process buildIndex {
 ...
 /*
 * Step 5. Trim reads with skewer (optional). Make empty channels for multiQC
 */
process trimReads {

    tag { pair_id }
    label 'big_comp'
	container 'biocorecrg/skewer:0.2.2'
    ...
    output:
    set pair_id, file("*trimmed*.fastq.gz") into trimmed_reads_for_mapping
    file("*trimmed*.fastq.gz") into filtered_read_for_QC
    file("*trimmed.log") into logTrimming_for_QC
     
    script:
    def infiles = reads.join(' ')
	"""
	skewer  -t ${task.cpus} -n -u -o ${pair_id} -z ${infiles}
	"""
}
...
process fastqcTrim {
    ...
    script:
 	"""
 	fastqc -t ${task.cpus} ${filtered_read}
 	"""
 }

/*
 * Align reads by using STAR mapper. Make new channels depending on whether you choose to filter or not and to have a SE or a PE analysis
*/

if (params.trimming == "YES")  {
    reads_for_mapping = trimmed_reads_for_mapping
    println("Trimming raw reads and mapping the trimmed ones")
} else {
    println("No trimming. Mapping directly the raw reads")
    reads_for_mapping = raw_reads_for_mapping
}
        ...
        if [ `echo "${reads}"| cut -f 1 -d " " | grep ".gz"` ]; then gzipped=" --readFilesCommand zcat "; else gzipped=""; fi
            STAR --genomeDir ${STARgenome} \
                 --readFilesIn ${reads} \
                  ...
                  --alignIntronMax 1 --alignMatesGapMax 500 \
...
process qualimap {
    ...
    if (params.strandness=="unstranded") {
        strandness = "non-strand-specific"
    } else if (params.strandness=="forward") {
        strandness = "strand-specific-forward"
    } else if (params.strandness=="reverse") {
         strandness = "strand-specific-reverse"
    }

    def qc = new QualityChecker(input:bamfile, annotation_file:annotation_file, output:"QUALIMAP_${pair_id}", strand:strandness, memory:qualimp_memory, mode:qualimode, extrapars:"-s")
    qc.qualimapRNAseq()  
 }
...
process tool_report {
        ...
        script:
        """
        make_tool_desc_for_multiqc.pl -l fastqc,star,skewer,samtools,htseq > tools_mqc.txt
        """
}
...
process multiQC_report {
    ...
    file '*' from Aln_folders_for_multiqc.mix(raw_fastqc_files, trimmed_fastqc_files, counts_stats_for_report,logTrimming_for_QC).flatten().collect()
