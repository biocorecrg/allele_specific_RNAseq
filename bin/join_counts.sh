...
awk -v fname=$1 'BEGIN {alla=allb=unknown=ambs=0} {if ($0!~"_" && $1!="gene_id") {alla+=$2; allb+=$3; unknown+=$4; ambs+=$5; }} END {print fname"\t"alla"\t"allb"\t"unknown"\t"ambs}' ${1}_group.counts >> ${1}_group.stats
